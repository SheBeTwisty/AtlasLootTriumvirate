...
function AtlasLoot_GetLODModule(dataSource)
	...
	elseif (dataSource=="AtlasLootTriumvirate") then
		return "AtlasLoot_Triumvirate";
	end
end
...
function AtlasLoot_LoadAllModules()
	local orig, bc, wotlk, craft, world, triumv;
	...
	triumv, _ = LoadAddOn("AtlasLoot_Triumvirate");
    local flag=0;
	if not orig then
	...
	if not triumv then
		LoadAddOn("AtlasLoot_Triumvirate");
		flag=1;
	end
	if flag == 1 then
		if ATLASLOOT_DEBUGMESSAGES then
